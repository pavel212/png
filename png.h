...
typedef enum {
  ...
  PNG_ALPHA = 2,                                 //16
  PNG_16BIT = 4,                                 //16
  PNG_GRAYALPHA = PNG_GRAY | PNG_ALPHA,          //16
  PNG_GRAY16 = PNG_GRAY | PNG_16BIT,             //16
  PNG_GRAYALPHA16 = PNG_GRAYALPHA | PNG_16BIT,   //32
  PNG_RGBA = PNG_RGB | PNG_ALPHA,                //32
  PNG_RGB16 = PNG_RGB | PNG_16BIT,               //48
...
static uint32_t crc32(uint32_t crc, const uint8_t *data, uint32_t num){
  while (num--) crc = crc_table[(crc ^ *data++)&0xFF] ^ (crc >> 8); 
  return crc;
}

static void crc32adler(uint32_t *crc, uint32_t *adler, const uint8_t *data, uint32_t num){
  uint32_t a = (*adler) & 0xFFFF, b = (*adler)>>16, c = *crc;
  while (num--){
    a = (a + *data) % 65521;
    b = (b + a) % 65521;
...
int png_write(const char *fn, const uint8_t *data, uint32_t width, uint32_t height, png_type type, const char **text){
  ...
  const uint64_t row_bytes  = width * (1+2*rgb+alpha) * bpp >> 3,
  ...
  #define FWRITEC(f,...) do{ const uint8_t d[] = {__VA_ARGS__}; if (sizeof(d) != fwrite(d, 1, sizeof(d), f)) return fclose(f),-2; crc = crc32(crc, d, sizeof(d)); } while(0)

  FILE *f = fopen(fn, "wb");
  if (!f) return -1;

  if (!crc_table[1]) for (uint32_t c,i=0; (c=i) < 256; crc_table[i++] = c) for (uint8_t j = 0; j < 8; j++) c = (0xedb88320*(c&1))^(c>>1);

  FWRITE_(f, 0x89, 'P', 'N', 'G', 0x0D, 0x0A, 0x1A, 0x0A);  
  ...
  crc = 0x13E5812D;                                                   //crc of 'I', 'D', 'A', 'T', 0x78, 0x01
  while (num = data_bytes - y*(row_bytes+1) - x){                     //remaining bytes to write
    if (pos + num > 65535)     num = 65535 - pos; else lastblock = 1; //block size is limited to 16 bits
    ...
    const uint8_t *p = &data[x+y*row_bytes];
    if (num != fwrite(p, 1, num, f)) return fclose(f),-2;
    crc32adler(&crc, &adler, p, num);
    if ((pos += num) >= 65535    ) {pos = 0;      }  //block complete
