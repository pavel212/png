...
Just to store raw data of 1,2,3,4,6,8 bytes per sample in a png "container". Or so small images, that compression does not make much sense[^1].
...
void main(){
  ...
  png_write("2x2.png", data, 2, 2, PNG_RGB, (char*[]){"Title\0title", "Comment\0comment", 0});
}
```
...
# png_write
```C
int png_write(const char *fn, const void *data, uint32_t width, uint32_t height, uint32_t type, const char **text);
```
